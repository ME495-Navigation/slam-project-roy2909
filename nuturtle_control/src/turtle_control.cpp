...
private:
  ...
  turtlelib::WheelPos wheel_vels_;
  ...
  void cmd_vel_cb(const std::shared_ptr<geometry_msgs::msg::Twist> msg)
  {
    ...
    } else if (wheel_vels_.left < -motor_cmd_max_) {
      wheel_vels_.left = -motor_cmd_max_;
    }

    if (wheel_vels_.right > motor_cmd_max_) {
      wheel_vels_.right = motor_cmd_max_;
    } else if (wheel_vels_.right < -motor_cmd_max_) {
      wheel_vels_.right = -motor_cmd_max_;
    }
